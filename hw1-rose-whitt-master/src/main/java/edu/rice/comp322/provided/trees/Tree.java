    ...
    class Node<T> implements Tree<T> {
        ...
        public <U> U fold(U zero, BiFunction<U, T, U> operator) {
            if (!this.children.isEmpty()) {
                var firstKid = this.children.head();
                var appendThis = this.children.tail();
                var subtree = makeNode(firstKid.value(), firstKid.children().appendAll(appendThis));
                return subtree.fold(operator.apply(zero, this.value()), operator);
            } else {
                return operator.apply(zero, this.value());
            }
        }
